...
protected:
    ...
    // The initial state of the analysis
        ...
        // Refer to this :
        // https://en.wikipedia.org/wiki/Data-flow_analysis#Forward_Analysis
  ...
  public:
    ...
    void print() {
			for (auto const &it : EdgeToInfo) {
        // if(IndexToInstr[it.first.first] != 0) {
        //   IndexToInstr[it.first.first]->print(errs());
        // }
        // errs() << "\n";
    ...
    void runWorklistAlgorithm(Function * func) {
      ...
      for(auto it = InstrToIndex.begin(), iend = InstrToIndex.end() ; it != iend ; it++){
      ...
      while(!worklist.empty()){
        unsigned instrIdx = worklist.back();
        ...
        // errs() << "working on instr: " << instrIdx << "\n";
        // IndexToInstr[instrIdx]->print(errs());
        // errs() << "\n";

        std::vector<unsigned> incoming, outgoing;
        std::vector<Info*> flowResult;

        getIncomingEdges(instrIdx, &incoming);
        getOutgoingEdges(instrIdx, &outgoing);
        flowfunction(IndexToInstr[instrIdx], incoming, outgoing, flowResult);
        
        for(int i = 0, size = outgoing.size() ; i < size ; i++){

          Edge currentEdge = Edge(instrIdx, outgoing[i]);

          // errs() << instrIdx << "->" << outgoing[i] << "\n";
          // errs() << "\n";
          // IndexToInstr[outgoing[i]]->print(errs());
          // for(auto it = EdgeToInfo.begin(), iend = EdgeToInfo.end() ; it != iend ; it++){
          //   errs() << it->first.first << " " << it->first.second << "\t";
          //   errs() << it->second->definedInstructions.size() << "\n";
          // }
          // errs() << "edge start" << "\n";
          // errs() << "edge: " << currentEdge.first << "," << currentEdge.second << "\n";
          // errs() << "edge size = " << EdgeToInfo[currentEdge]->definedInstructions.size() << "\n";
          // errs() << "flowResult = " << flowResult[i]->definedInstructions.size() << "\n";
          // errs() << "equals: " << Info::equals(EdgeToInfo[currentEdge], flowResult[i]) <<  "\n";

          if(! Info::equals(EdgeToInfo[currentEdge], flowResult[i]) ){
            if(! Info::equals(EdgeToInfo[currentEdge], &Bottom)){
                delete EdgeToInfo[currentEdge];
            }
            EdgeToInfo[currentEdge] = flowResult[i];
            worklist.push_front( outgoing[i] );
          }else{
            delete flowResult[i];
          }
        }
